...
/*
  void UpdateRuns(Statistable::TypeTime now) {
    ...
    double TGet = 1.0 * stats->GetStatistics(KSGetCount, now - 1) / time;
    double TWrite = 1.0 * stats->GetStatistics(KSPutCount, now - 1) / time;
    double TIterate = 1.0 * stats->GetStatistics(KSIterateCount, now - 1) / time;
    int64_t TLeaf = stats->GetStatistics(LeafCount, -1);
    
    std::vector<SBSNode*> children;
    SBSNode* tail = Current().Next();

    LevelNode::VariableTable& table = Current().Table();
    for (SBSNode* n = Current().node_; n != tail; n = n->Next(height - 1)) {
      const Statistics* stats = n->GetTreeStatistics(height); 
      auto& ctable = n->GetLevel(height - 1)->table_;
      double CGet = 1.0 * stats->GetStatistics(KSGetCount, now - 1) / time;
      double CWrite = 1.0 * stats->GetStatistics(KSPutCount, now - 1) / time;
      double CIterate = 1.0 * stats->GetStatistics(KSIterateCount, now - 1) / time;
      int64_t CLeaf = stats->GetStatistics(LeafCount, -1);
      if (height == 1) {
        size_t PageSize = 4096;
        // The average number of copies held per key, equal to write amplification.
        double CReplica = stats->GetStatistics(KSAmplification, now - 1) / time;
        // The number of read operations generated by 1 standard size file in 1 second.
        ctable.IPF = CGet * PageSize / CReplica;
        // 1 / How many read operations will result in one disk read.
        //double IPR_child = table.IPR;
      
      }
      
      {
        // The incremental read bandwidth generated by 1 standard size file in 1 second.
        double IPF_child = ctable.IPF;
        // The incremental write bandwidth generated by 1 standard size file in 1 second.
        double OPF_child = ctable.OPF;

        table.IPF += IPF_child * CGet / TGet;
        table.OPF += OPF_child * CWrite / TWrite;
      }
    }
  }
  */
