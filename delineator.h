...
struct Delineator {
 ...
 public:
  Delineator() : vec_() {}
  virtual ~Delineator() {}
  void AddStatistics(const Slice& guard, std::shared_ptr<Statistable> stats) { vec_.emplace_back(guard, stats); }
  int64_t GetStatistics(size_t k, uint32_t label, int64_t time) { return vec_[k].stats_ ? vec_[k].stats_->GetStatistics(label, time) : 0; }
  void GetAllStatistics(std::vector<int64_t>& data, uint32_t label, size_t time) {
    for (size_t i = 0; i < vec_.size(); ++i)
      data.push_back(GetStatistics(i, label, time));
  }
  void PrintTo(std::ostream& os, int64_t time, DefaultTypeLabel label, size_t height = 10, size_t width = 60) {
    size_t n = vec_.size();
